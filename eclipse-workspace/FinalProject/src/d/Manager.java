...
public class Manager extends Employee implements Orders { 
	...
	public boolean checkMarkTime(Student s) throws ParseException {
		...
		if(type == ManagerTypes.OR) {
				...
				HashMap<Course,Mark> map =s.Courses;
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				 Date d1;
						for (Entry<Course, Mark> entry : map.entrySet())
						{
						    if(Login.semester.equals("Fall") ) {
						    	d1 = sdf.parse("2017-12-20");
						    	if(d1.compareTo(entry.getValue().getTimeOfPutting())<0) {
						    		inTime=false;
						    		System.out.println("Teacher "+entry.getKey().teacher.toString()+
